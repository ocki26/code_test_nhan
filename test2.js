...
function parseProxyConfig(raw) {
  ...
  if (parts.length === 4) {
    ...
    return {
      ...
      user,
      pwd,
      // Playwright chỉ nhận server, phần auth sẽ do Extension xử lý
      pwServer: `socks5://${ip}:${port}`,
      // URL check IP cho axios
      checkUrl: `socks5://${user}:${pwd}@${ip}:${port}`,
    };
  }
  return null;
}
...
// ==============================================================================
// 2. TẠO EXTENSION XỬ LÝ AUTH (FIX LỖI SOCKS5 AUTH)
// ==============================================================================
function createProxyAuthExtension(host, port, user, pass) {
  ...
  // 2. Background Script (Tự động điền User/Pass)
  const backgroundJs = `
        const config = {
            mode: "fixed_servers",
            rules: {
        ...
        // Chrome không cho set SOCKS5 auth trực tiếp, ta dùng sự kiện onAuthRequired
        ...
        // (Tùy chọn) Force set proxy qua API của Chrome luôn cho chắc
...
async function getProxyInfo(proxyUrl) {
  console.log(`[*] Checking Proxy: ${proxyUrl.split("@")[1]}...`);

  // Tự động chọn Agent (SOCKS hoặc HTTP)
  let agent;
  ...
  // Chỉ check 1 API cho nhanh vì ở trên bạn đã check OK rồi
  try {
...
async function run() {
  ...
  if (!realIp) {
    console.log("[!!!] Proxy chập chờn, nhưng vẫn cố gắng chạy...");
    realIp = "127.0.0.1";
  ...
  // --- TẠO EXTENSION ---
  console.log("[*] Đang tạo Extension xử lý Auth...");
  ...
  const ruyiConfig = {
    ...
    webrtc_public_ip: realIp, // Inject IP
    net_downlink: 10.0,
    net_rtt: 50,
    dnt: "1",
    noise_seed: 12345,
  ...
  try {
    console.log(`[*] Mở Browser (Proxy do Extension quản lý)...`);
    ...
    const context = await chromium.launchPersistentContext(userDataDir, {
      ...
      args: launchArgs,
      viewport: null,
      // QUAN TRỌNG: KHÔNG TRUYỀN PROXY VÀO ĐÂY NỮA
      // proxy: ..., (Bỏ dòng này, để Extension tự bắt connection)
      locale: "en-US",
